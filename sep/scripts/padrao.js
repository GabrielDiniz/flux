...
function excluirCategoria(num){
	document.getElementById('txtNum').value = num;
	document.getElementById('frmInsereCat').action = './inc/servicos/categoria_excluir.php';	
	document.getElementById('frmInsereCat').submit();
}
...
function DivManipulavel(){
	var fundo = document.getElementById('fundoDiv');
	var conteudo = document.getElementById('conteudoDiv');
	var displayfundo='block';
	var displayconteudo='block';
	if(fundo.style.display =='block'){
		
		displayfundo='none'
		displayconteudo='none'
	}
	fundo.style.display=displayfundo;
	conteudo.style.display=displayconteudo;
} 
...
$(document).ready(function(){
	$('#btInformar').click(function(){		
	    $('#divLivro').html('<img src="../img/loader.gif" alt="Carregando" />');	
		$.ajax({
			type: "POST",
			url: "../livro/sep_validacontribuinte.ajax.php",
			data: $('#FrmLivro').serialize(),
			success: function(msg){
			  $('#divLivro').html(msg);		
			}
	    });								
	...
	$('#txtCnpjLivro').blur(function(){
		$('#btInformar').trigger('click');
	});
	
	$('body').delegate('#txtDataVencimento','keyup',function() {MaskData(this);});
	...
	var calendarioBR={
		...
		if(document.getElementById('txtDataInicial')){
			$("#txtDataInicial").datepicker(calendarioBR);
			$("#txtDataFinal").datepicker(calendarioBR);
		}
...
function limitePct(idinput){
	var valor = document.getElementById(idinput).value;
	if(valor > 100){
		alert("Não é possivel ultrapassar 100%!");
		return false;
	}
}

function contaTeclas(input,tipo,retorno,codtipodec){
	var campo = document.getElementById(input);
	var valor = campo.value;
	document.getElementById('btVoltar').style.display = 'none';
	ajax({
		url:'inc/declarar_infos.ajax.php?txtCNPJ='+valor+'&tipo='+tipo+'&codtipodec='+codtipodec+'&a=a',
		espera: function(){
...
function verificaPrestador_guias(input,tipo,retorno){
	var campo = document.getElementById(input);
	var valor = campo.value;
	if(valor){
		if((valor.length == 14) || (valor.length == 18)){
			document.getElementById('btVoltar').style.display = 'none';
			ajax({
				url:'inc/declaracoes/'+tipo+'/guias/guia_pagamento.php?txtCNPJ='+valor+'&a=a',
				espera: function(){
					document.getElementById(retorno).innerHTML = 'Verificando...';
				},
				sucesso: function(){
			...
			alert("O número informado está em um formato inválido!");
		}
	}else{
		alert("É necessário digitar um cnpj!");	
	}
}
...
function buscaCidadesConfiguracao(campo, resultado, cadastro) {
	if(cadastro === undefined) cadastro = true;
	
	if(campo.value!=''){
		var url = cadastro ? 'inc/listamunicipioConfiguracoes.ajax.php?UF='+campo.value :'../inc/listamunicipio.ajax.php?UF='+campo.value;
		
		ajax({
			url:url,
			espera: function(){
				document.getElementById(resultado).innerHTML = '<select name="txtInsMunicipioEmpresa" style="width:150px;"><option/></select>';
			},
			sucesso: function() {
				document.getElementById(resultado).innerHTML = respostaAjax;
			},
...
{
	var targtemp = 	document.getElementById(formid).target;
	var actiontemp = document.getElementById(formid).action;

	document.getElementById(formid).target = formtarget;
	document.getElementById(formid).action = formaction;		
	document.getElementById(formid).submit();
	
	document.getElementById(formid).target = targtemp;
	document.getElementById(formid).action = actiontemp;
}
...
function VisualizarNovaLinha(cod,area,obj,url){
	if(hdtd){         										//verifica se o hidden da td ja tem valor
		if(hdtd == area){
			area = '';
			if(document.getElementById(hdtd)){
				document.getElementById(hdtd).innerHTML = '';	//define que o elemento do HTML receba vazio para que nao haja mais de um aberto
			}
		}else{
...
function addLinhaTomador(){
    var quantidade = document.getElementById('hdInputs').value;
	quantidade++;
	var codemissor = document.getElementById('hdCodemissor').value;	
	var ultimaLinha = document.getElementById('retornoDivTomador').rows.length;
	var novaLinha   = document.getElementById('retornoDivTomador').insertRow(ultimaLinha);
	var novaColuna  = novaLinha.insertCell(0);
	...
	//Inseri mais uma linha
	ajax({
		url:'../site/linha_nova_nfe/tomador.php?quantidade='+quantidade+'&codemissor='+codemissor+'&a=a',
		sucesso: function(){
			novaColuna.innerHTML = respostaAjax;	
			document.getElementById('hdInputs').value = quantidade;
...
function mostraDivTomador(){
	document.getElementById('divTomadorNota').style.display = 'block';
}
