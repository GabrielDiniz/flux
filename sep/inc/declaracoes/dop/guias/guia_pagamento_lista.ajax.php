...
$sql = mysql_query("
	...
	WHERE 
		MONTH(dop_des.competencia) = '$mes' AND 
		YEAR(dop_des.competencia) = '$ano' AND 
		dop_des.codorgaopublico = '$codemissor' AND 
		dop_des.estado = 'N'
	GROUP BY 
		dop_des.codigo
");
			   ...
			   <div style=" width:70%; <?php if(mysql_num_rows($sql)>13){ echo "height:300px; overflow:auto";}?>">
						...
						while(list($codigo,$data,$codverificacao,$total,$data_comp) = mysql_fetch_array($sql)){
							...
							?>
								<tr bgcolor="#FFFFEA">
									...
									<td align="center" width="1">
										<input type="checkbox" name="ckISS<?php echo $cont; ?>" id="ckISS<?php echo $cont; ?>" value="<?php echo "$total|$codigo"; ?>" onclick="GuiaPagamento_SomaISS(this)" />
				...
				<table>
					<tr>
						<td>Total</td>
						<td>
							<input type="text" class="texto" name="txtTotalIss" id="txtTotalIss" style="text-align: right;" value="0,00" readonly="readonly" >
						</td>
					</tr>
					<tr>
						<td>
