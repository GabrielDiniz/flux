...
<?php
	...
	if($cancelaDes){
		mysql_query("UPDATE simples_des SET estado = 'C' WHERE codigo = '$cancelaDes'");
		addlogs('Atualizou uma Declaração Simples');
	}//fim if cancela
	
	//verifica quais campos foram preenchidos e concatena na variavel str_where
	if($compmes){
		$str_where = " AND MONTH(simples_des.competencia) = '$compmes'";
	}
	if($compano){
		$str_where .= " AND YEAR(simples_des.competencia) = '$compano'";
	}
	if($data){
		$data = DataMysql($data);
		$str_where .= " AND simples_des.data_gerado = '$data'";
	}
	if($cnpj){
		$str_where .= " AND cadastro.cnpj = '$cnpj'";
	}
	if($estado){
		$str_where .= " AND simples_des.estado = '$estado'";
	}
	if($numero){
		$str_where .= " AND simples_des.codigo = '$numero'";
	}
...
<td bgcolor="#FFFFFF" align="left" colspan="2">&nbsp;
        	<label title="Ver Detalhes">
            	...
            	onClick="VisualizarNovaLinha('<?php echo $dados_pesquisa['codigo'];?>','<?php echo"tddes".$x;?>',this,'inc/declaracoes/simples/declarar_vizualizar.ajax.php')">
            </label>
            &nbsp;
            ...
            <label title="Cancelar Declaração" id="lbCancelar<?php echo $x;?>">
                ...
                onClick="document.getElementById('hdPrimeiro').value=1; return cancelarDeclaracao('<?php echo $dados_pesquisa['codigo'];?>','<?php echo $dados_pesquisa['nome'];?>','inc/declaracoes/simples/declarar_pesquisa.ajax.php','frmDes','divDeclaracoesSimples','hdCancelaDes');">
            </label>
            <?php }?>
        </td>
    </tr>
