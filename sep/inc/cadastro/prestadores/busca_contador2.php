		...
		<td align="center">
			<form method="post"  name="frmbuscacont" id="frmbuscacont">
				...
				<table width="100%">
					...
					<tr>
					  <td>
						<input name="txtBuscaCPFCNPJ" id="txtBuscaCPFCNPJ" type="text" class="texto" size="39" 
							style="background-color:#255b8f; color:#FFFFFF; text-transform:uppercase" 
							onkeydown="return NumbersOnly( event );" onkeyup="CNPJCPFMsk( this );">
						<input name="btBuscarContador" type="submit" value="" id="btBuscarCliente" title="Buscar">
						</td>
                    ...
                    <tr>
                        <td colspan="2">
							<input name="txtBuscaRazao" id="txtBuscaRazao" type="text" class="texto" size="39" 
							style="background-color:#255b8f; color:#FFFFFF; text-transform:uppercase">
						</td>
					</tr>
					<tr>
						<td  align="center">
							...
							<?php 
							if(isset($_POST['txtBuscaRazao']))
							{
								$nome=$_POST['txtBuscaRazao'];					
								$cpfcnpj = $_POST['txtBuscaCPFCNPJ'];
								$campo = tipoPessoa($cpfcnpj);
								
								if($nome != '' && $cpfcnpj != ''){ //Se vier ambos preenchidos
									$where=" AND razaosocial LIKE'%$nome%' AND $campo = '$cpfcnpj' AND estado <> 'NL'";
								}
								
								if($nome != '' && $cpfcnpj == ''){ //Se vier só o nome preenchido
									$where=" AND razaosocial LIKE'%$nome%' AND estado <> 'NL'";
								}
								
								if($nome == '' && $cpfcnpj != ''){ //Se vier só o cpf/cnjp preenchido
								...
								$sql=mysql_query("
										SELECT 
											...
											nome, 
											razaosocial,
											IF(cnpj <> NULL,cnpj,cpf) AS cnpjcpf,
											inscrmunicipal,						
											logradouro,
											numero, 
											municipio, 
