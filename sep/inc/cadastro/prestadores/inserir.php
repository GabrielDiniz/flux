...
$include=$_POST['include'];
	...
	$sql_categoria=mysql_query("SELECT codigo FROM servicos_categorias WHERE nome = 'Contï¿½bil'");	
	...
	if(mysql_num_rows($teste_nome)>0){
	   ...
	   //Pega os codtipo dos prestadores que tem informaï¿½ï¿½es extras
	   $codtipo_inst = codtipo('instituicao_financeira');
	   $codtipo_opr  = codtipo('operadora_credito');
	   $codtipo_cart = codtipo('cartorio');
	   
	   //testa se o prestador que estï¿½ sendo editado tem alguma informaï¿½ï¿½o extra
	   if($codtipo == $codtipo_inst){
			...
			add_logs('Inseriu uma Instituiï¿½ï¿½o Financeira');
			...
			$codcargo = codcargo("Gerente");
			add_logs('Inseriu uma Operadora de Crï¿½dito');
			mysql_query("INSERT cadastro_resp SET nome = '$gerente', cpf = '$gerente_cpf', codcargo = '$codcargo', codemissor = '$codigoempresa'");
	   ...
	   //insere os socios no banco
		while($contsocios < $nrosocios) {   
			if($vetor_sociosnomes[$contsocios] != "") {
                if($contsocios==0){
                    $sql_cargo=mysql_query("SELECT codigo FROM cargos WHERE cargo='Responsï¿½vel'");
                }else{
                    $sql_cargo=mysql_query("SELECT codigo FROM cargos WHERE cargo='Sï¿½cio'");
                }
