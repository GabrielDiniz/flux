...
?>
    ...
    ?>
        ...
        <title>Imprimir Al&iacute;quota Simples</title>
    ...
    <body>
        ...
        <table width="95%" border="2" cellspacing="0" class="tabela">
          <tr>
            ...
            <td width="584" height="33" colspan="2">
              <span class="style1">
                  <center>
                     <p>RELAT&Oacute;RIO DE <b>AL&Iacute;QUOTAS DO SIMPLES UTILIZADAS PELO CONTRIBUINTE</b> </p>
        ...
        <?php
            ...
            $where .= " AND cadastro.nfe = 'S' AND cadastro.codtipodeclaracao = 3";
            $sqlValores = mysql_query("
               ...
               FROM 
			   	   notas INNER JOIN cadastro ON notas.codemissor = cadastro.codigo
               $where
           ");
        ...
        <table width="95%" border="1" cellspacing="0" class="tabelameio"  >
            <tr>
                ...
                <td valign="top">
                   <?php
                       echo "<b>Média al&iacute;quota percentual:</b> ".DecToMoeda($valores->aliqpercentual);
                   ?>
                </td>
                <td>
                    <?php
                       echo "<b>Média al&iacute;quota INSS:</b> ".DecToMoeda($valores->aliqinss);
                    ?>
                </td>
                <td>
                    <?php
                       echo "<b>Média al&iacute;quota IRRF:</b> ".DecToMoeda($valores->aliqirrf);
                    ?>
                </td>
            </tr>
        </table>
        <?php
            ...
            $query = ("
                SELECT
					...
					notas.numero,
                    notas.aliq_percentual AS aliqpercentual,
                    notas.aliqinss AS aliqinss,
                    notas.aliqirrf AS aliqirrf
                FROM 
				    notas INNER JOIN cadastro ON notas.codemissor = cadastro.codigo
                $where
            ...
            ?>
                <table width="95%" class="tabela" border="1" cellspacing="0">
                    <tr style="background-color:#999999">
                      ...
                      <td align="center"><strong>CNPJ / CPF</strong></td>
                      <td align="center"><strong>N&ordm; Nota</strong></td>
                      <td align="center"><strong>Al&iacute;quota percentual</strong></td>
         ...
         ?>
            <tr id="trDecc<?php echo $x;?>">
                ...
                <td bgcolor="white" align="center"><?php echo DecToMoeda($dados_pesquisa['aliqpercentual']);?></td>
                <td bgcolor="white" align="center"><?php echo DecToMoeda($dados_pesquisa['aliqinss']);?></td>
                <td bgcolor="white" align="center"><?php echo DecToMoeda($dados_pesquisa['aliqirrf']);?></td>
