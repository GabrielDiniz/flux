...
<form method="post"  name="frmbusca" id="frmbusca">
	...
	<table width="100%">        
        ...
        <tr>        	
			<td>
            	<input name="txtBuscaCPFCNPJ" id="txtBuscaCPFCNPJ" type="text" class="texto"  onkeydown="return NumbersOnly( event );" onkeyup="CNPJCPFMsk( this );">	
			</td>            
	  	</tr>
        <tr>        	
			<td colspan="2"> 
        ...
        <tr>        	
			<td colspan="2">
            	<input name="txtBuscaNome" id="txtBuscaNome" type="text" class="texto" size="59"  text-transform:uppercase"  >	
			</td>
			...
			if(isset($_POST['txtBuscaNome']))
				{
					...
					if($nome != '' && $cpfcnpj == ''){
						$where=" WHERE nome LIKE'%$nome%' AND estado <> 'NL' AND codtipodeclaracao = 3";
					}
					else if($nome == '' && $cpfcnpj != ''){
						$where=" WHERE $campo LIKE'%$cpfcnpj%' AND estado <> 'NL' AND codtipodeclaracao = 3";
					}
					else{
						$where=" WHERE codtipodeclaracao = 3";
					}
