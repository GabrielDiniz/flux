...
function AbrirGuias(ano, mes, tipo, codigo){
//$('#divResultado').html('carregando');	
	$.ajax({			
		url: "./inc/guia_pagamento.ajax.php?cmbAno=" + document.getElementById(ano).value + "&cmbMes=" + document.getElementById(mes).value + "&cmbEmpresaCliente=" + document.getElementById('cmbEmpresaCliente').value,
		...
		<td height="60" colspan="3"  align="center">
			...
			<form method="post" id="frmPeriodo">
				...
				<table>
					<tr>
						<td>
                    <input type="hidden" name="cmbEmpresaCliente" id="cmbEmpresaCliente" value="<?php echo $_POST['cmbEmpresaCliente']; ?>" />
                    ...
                    <input type="hidden" name="stringsql" id="stringsql" value="<?php echo " AND estado='N'"; ?>" />
                      ...
                      <select name="cmbMes" id="cmbMes">
						...
						<td><input type="submit" class="botao" name="btBuscar" value="Buscar" onclick="AbrirGuias('cmbAno','cmbMes','cmbTipo','cmbEmpresaCliente'); return false;"/></td>
					</tr>
