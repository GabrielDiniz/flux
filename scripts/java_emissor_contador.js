...
function validaAliquota(number){
    ...
    if(aliquota < 2 || aliquota > 5){
        alert(utf8_decode('A alÃ­quota não deve ser maior que 5,00% nem menor que 2,00%'));
...
// FUNï¿½ï¿½ES da GUIA DE PAGAMENTO
...
function ValidaCkbDec(campo){
	...
	if(total>0){
		...
		alert(utf8_decode('Ã‰ necessário que escolha ao menos uma declaração'));
		return false;
	}
...
// FUNï¿½ï¿½ES da GUIA DE PAGAMENTO fim
...
function ValorIss(regras_de_credito)
{
	...
	if((tipopessoa == 14) || (tipopessoa == 18)){
		if(basecalc != ""){
			...
			//calcula o crï¿½dito final que o tomador receberï¿½ ao emitir a nota
...
function ValorIssRPA(cred_pf_n,val_pf_n,cred_pf_s,val_pf_s,cred_pj_n,val_pj_n,cred_pj_s,val_pj_s)
{
	...
	if(document.getElementById('txtPissretido').value>100){ 
		...
		alert(utf8_decode('Não é possÃ­vel reter mais de 100% de ISS'));
	}
	 else{
	 var credito_final;	
	 var credito = 0;	 
	 ...
	 if((tipopessoa == 14) || (tipopessoa == 18))
	 {
		 ...
		 if(aliquota != "")
		 { 
			  //separa os valores do combo e pega o valor do crï¿½dito
			  ...
			  if(basecalc != "")
			  {
			   ...
			   var iss = parseFloat(aliquota);  //rpa o calculo ï¿½ direto e nao por porcetagem 

			   /*a = Math.sqrt(iss);*/
			   iss = iss.toFixed(2);
			   document.getElementById('txtISS').value=DecToMoeda(iss);	  
			  
				  //verifica a quantidade de crï¿½ditos que o tomador receberï¿½, baseando-se no tipo de pessoa e se tem iss retido ou nï¿½o.
				  if( tipopessoa == 14)
				   {	   
					 if (issretido[2] != 0)
					 {
			...
			//calcula o valor do ISS que serï¿½ retido
			...
			total = (total) + parseFloat(iss); // var iss ï¿½ do valor do rpa que deve ser somado com o valor total da nota
...
//funï¿½ï¿½o genï¿½rica que requisita confirmaï¿½ï¿½o de envio
		...
		if((base!="")&&(inss!=""))
			{
				...
				else{alert(utf8_decode('Não é possÃ­vel reter um valor de INSS maior que 100%'));}	
			}
	}*/
...
function CalculaISS(){
	...
	if((base!="")&&(iss!="")){
		if (iss<=100){
			...
			alert(utf8_decode('Não é possÃ­vel reter um valor de ISS maior que 100%'));
		}	
	}else{
		document.getElementById('txtBaseCalculo').value = DecToMoeda(base);	
...
function CalculaIR(){
	...
	if((base!="")&&(ir!="")){
		if (ir<=100){
			...
			alert(utf8_decode('Não é possÃ­vel reter um valor de IR maior que 100%'));
		}	
	}else{
		document.getElementById('txtBaseCalculo').value = DecToMoeda(base);	
		document.getElementById('txtBaseCalculoAux').value = DecToMoeda(base);
