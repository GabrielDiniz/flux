...
$hoje=date("Y-m-d");
$sql=mysql_query("SELECT codigo, valorisstotal, geracao, periodo, vencimento FROM livro WHERE SUBSTRING(periodo,1,4)='$ano' AND SUBSTRING(periodo,6,2)='$mes' AND valoriss > 0 AND codcadastro='$codcadastro' AND estado='N'") or die(mysql_error());
...
<form method="post" id="frmGuia">	
    ...
    <input type="hidden" name="txtEmissor" value="<?php echo $codcadastro;?>" />
			<table border="0" width="100%">
			    ...
			    <?php
				while(list($codigo,$total,$data,$competencia,$vencimento)=mysql_fetch_array($sql)){
