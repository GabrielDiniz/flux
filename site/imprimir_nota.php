...
list($codigo, $numero, $codverificacao, $datahoraemissao, $rps_numero, $rps_data, $tomador_nome, $tomador_cnpjcpf, $tomador_inscrmunicipal, $tomador_inscrestadual, $tomador_endereco, $tomador_logradouro, $tomador_numero, $tomador_complemento, $tomador_cep, $tomador_municipio, $tomador_uf, $tomador_email, $discriminacao, $valortotal, $estado, $credito, $valordeducoes, $basecalculo, $valoriss,$valorinss,$aliqinss,$valorirrf,$aliqirrf, $deducao_irrf, $total_retencao, $motivo_cancelamento, $empresa_razaosocial, $empresa_nome, $empresa_cnpj, $empresa_cpf, $empresa_inscrmunicipal, $empresa_inscrestadual, $empresa_endereco, $empresa_numero, $empresa_municipio, $empresa_uf, $empresa_logo ,$issretido,$codtipo,$pispasep,$codtipodec,$observacao,$cofins,$contribuicaosocial) = mysql_fetch_array($sql);
		...
		<!-- tabela prefeitura fim -->	</td>
			<td width="25%" colspan="2" align="left" style="border:#000000 1px solid">N&uacute;mero da Nota<br /><div align="center"><font face="Verdana, Arial, Helvetica, sans-serif" size="3"><strong><?php print $numero; ?></strong></font></div></td>
		  </tr>
		  <tr>
			<td align="left" colspan="2" style="border:#000000 1px solid">Data e Hora de Emiss&atilde;o<br /><div align="center"><font face="Verdana, Arial, Helvetica, sans-serif" size="3"><strong><?php print (substr($datahoraemissao,8,2)."/".substr($datahoraemissao,5,2)."/".substr($datahoraemissao,0,4)." ".substr($datahoraemissao,11,2).":".substr($datahoraemissao,14,2)); ?></strong></font></div></td>
		  </tr>
		  <tr>
			<td align="left" colspan="2" style="border:#000000 1px solid">C&oacute;digo de Verifica&ccedil;&atilde;o<br /><div align="center"><font face="Verdana, Arial, Helvetica, sans-serif" size="3"><strong><?php print $codverificacao; ?></strong></font></div></td>
		  </tr>
		  <tr>
		...
		<table width="100%" border="0" cellspacing="0" cellpadding="2" align="center">
		  ...
		  <tr>
			<td align="left">&nbsp;&nbsp;Nome/Raz&atilde;o Social: <strong><?php print verificaCampo($tomador_nome); ?></strong></td>
		...
		<!-- tabela discrimacao dos servicos -->	
		  ...
		  <tr>
			...
			?>
			<td style="border:#000000 1px solid">Deduções (R$)<br /><div align="right"><strong><?php print DecToMoeda($valordeducoes); ?></strong></div></td>
			<td style="border:#000000 1px solid">Acréscimos (R$)<br /><div align="right"><strong><?php print DecToMoeda($valoracrescimos); ?></strong></div></td>
			<?php
				}else{
					if($ativar_creditos == "n"){
						$colspan = "colspan=\"3\"";
		  ...
		  <tr>
			...
			if($issretido != 0)
			{
			  echo "- Esta NF-e possui reten&ccedil;&atilde;o de ISS no valor de R$ ".DecToMoeda($issretido)."<br> ";
			
			}
			
			if($pispasep > 0){
				echo "- Est&aacute; NF-e possui PIS/PASEP no valor de R$ ".DecToMoeda($pispasep)."<br />";
			}
			
			if($cofins > 0){
				echo "- Est&aacute; NF-e possui COFINS no valor de R$ ".DecToMoeda($cofins)."<br />";
			}
			
			if($contribuicaosocial > 0){
				echo "- Est&aacute; NF-e possui Contribui&ccedil;&atilde;o Social no valor de R$ ".DecToMoeda($contribuicaosocial)."<br />";
			}
			...
			if(($CONF_CIDADE != $tomador_municipio) && ($codtipodec != $codtipoSN)) {
				if($ativar_creditos == "s"){
					echo "- Esta NF-e n&atilde;o gera cr&eacute;dito, pois o Tomador de Servi&ccedil;os est&aacute; localizado fora do munic&iacute;pio de $CONF_CIDADE<br>";
				}
			} // fim if	
			if($rps_numero){
			?>
			...
			if($valorinss > 0){ //soh mostra se tiver valor
				if($aliqinss > 0){
					...
					echo "- Reten&ccedil;&atilde;o de INSS com valor de R$ ".DecToMoeda($valorinss)." <br>";
				}
			}
			if($valorirrf > 0){ //soh mostra se tiver valor
				if($aliqirrf > 0){
					echo "- Reten&ccedil;&atilde;o de IRRF ".DecToMoeda($aliqirrf)."% com valor de R$ ".DecToMoeda($valorirrf).""; if($deducao_irrf > 0){ echo ". Dedução de R$ ".DecToMoeda($deducao_irrf); }
					echo "<br>";
				}else{
					echo "- Reten&ccedil;&atilde;o de IRRF com valor de R$ ".DecToMoeda($valorirrf).""; if($deducao_irrf > 0){ echo ". Dedu&ccedil;&atilde;o de R$ ".DecToMoeda($deducao_irrf); }
					echo "<br>";
				}
			}
			if($total_retencao > 0){
				echo "- Total de reten&ccedil;&otilde;es da nota R$ ".DecToMoeda($total_retencao)." <br>";
			}
			?>
			</td>
		  </tr> 
